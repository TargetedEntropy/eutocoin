...
The EutoCoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play eutocoins" on the test network, if you
...
EutoCoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- EutoCoinMiner : Generates eutocoins (if wallet is enabled).
...
However, in open source software such as EutoCoin Core, where everyone uses
...
pay attention to for reviewers of EutoCoin Core code.

General EutoCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: EutoCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of EutoCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against EutoCoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/eutocoin-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/eutocoin-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:eutocoin/eutocoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `eutocoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `eutocoin-cli` and the GUI debug console use this table to determine how to
