...
#include "config/eutocoin-config.h"
#endif

#include "eutocoingui.h"

#include "eutocoinunits.h"
...
const std::string EutoCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString EutoCoinGUI::DEFAULT_WALLET = "~Default";

EutoCoinGUI::EutoCoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void EutoCoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a EutoCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and eutocoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to EUTO addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your EutoCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified EutoCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a eutocoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible EutoCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void EutoCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void EutoCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** EUTO START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/eutocointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* eutocoinLabelLayout = new QVBoxLayout(toolbarWidget);
        eutocoinLabelLayout->addWidget(label);
        eutocoinLabelLayout->addWidget(toolbar);
        eutocoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        eutocoinLabelLayout->addStretch(1);
...
void EutoCoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool EutoCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool EutoCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void EutoCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void EutoCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void EutoCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void EutoCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void EutoCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EutoCoinGUI::optionsClicked()
{
...
void EutoCoinGUI::showDebugWindow()
{
...
void EutoCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void EutoCoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void EutoCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void EutoCoinGUI::openClicked()
{
...
void EutoCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void EutoCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void EutoCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void EutoCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void EutoCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void EutoCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** EUTO START */
void EutoCoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void EutoCoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void EutoCoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void EutoCoinGUI::gotoRestrictedAssetsPage()
{
...
void EutoCoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to EutoCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void EutoCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void EutoCoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void EutoCoinGUI::updateHeadersSyncProgressLabel()
{
...
void EutoCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void EutoCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("EutoCoin"); // default title
...
void EutoCoinGUI::changeEvent(QEvent *e)
{
...
void EutoCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void EutoCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void EutoCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "EUTO")
        msg += tr("Amount: %1\n").arg(EutoCoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(EutoCoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void EutoCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void EutoCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool EutoCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool EutoCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void EutoCoinGUI::setHDStatus(int hdEnabled)
{
...
void EutoCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void EutoCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void EutoCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void EutoCoinGUI::detectShutdown()
{
...
void EutoCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void EutoCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void EutoCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(EutoCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void EutoCoinGUI::subscribeToCoreSignals()
{
...
void EutoCoinGUI::unsubscribeFromCoreSignals()
{
...
void EutoCoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void EutoCoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<EutoCoinUnits::Unit> units = EutoCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const EutoCoinUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(EutoCoinUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(EutoCoinUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (EutoCoinUnits::Unit u : EutoCoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(EutoCoinUnits::name(u)), this);
...
{
    setText(EutoCoinUnits::name(newUnits));
}
...
void EutoCoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=EUTOBTC"));
