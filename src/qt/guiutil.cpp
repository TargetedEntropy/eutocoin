...
#include "eutocoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a EutoCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new EutoCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new EutoCoinAddressCheckValidator(parent));
}
...
bool parseEutoCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no eutocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("eutocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!EutoCoinUnits::parse(EutoCoinUnits::EUTO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseEutoCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert eutocoin:// to eutocoin:
    //
    //    Cannot handle this later, because eutocoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("eutocoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "eutocoin:");
    }
    QUrl uriInstance(uri);
    return parseEutoCoinURI(uriInstance, out);
}

QString formatEutoCoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("eutocoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(EutoCoinUnits::format(EutoCoinUnits::EUTO, info.amount, false, EutoCoinUnits::separatorNever));
        paramCount++;
    }
...
bool openEutoCoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(EUTOCOIN_CONF_FILENAME);
    ...
    /* Open eutocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "EutoCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "EutoCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("EutoCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for EutoCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "eutocoin.desktop";
    return GetAutostartDir() / strprintf("eutocoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a eutocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=EutoCoin\n";
        else
            optionFile << strprintf("Name=EutoCoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the eutocoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef eutocoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (eutocoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, eutocoinAppUrl);

    CFRelease(eutocoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add eutocoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, eutocoinAppUrl, nullptr, nullptr);
    }
