...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for EUTO
    ...
    if (p_rewardSnapshot.strDistributionAsset != "EUTO") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is EUTO with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using EUTO differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "EUTO") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse EutoCoin address (already validated during ownership snapshot creation)
