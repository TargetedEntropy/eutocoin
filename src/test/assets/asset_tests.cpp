...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of EUTOCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("EUTO", type));
        BOOST_CHECK(!IsAssetNameValid("EUTOCOIN", type));
        BOOST_CHECK(!IsAssetNameValid("EUTOCOIN", type));

        //- Versions of EUTOCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("EUTOCOIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("EUTOCOIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("EUTOSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEREUTO", type));
        BOOST_CHECK(IsAssetNameValid("EUTOCOINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEEUTOCOIN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_EUTOCOINS", type));
        BOOST_CHECK(IsAssetNameValid("SEEUTOOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("EUTOCOIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("EUTOCOIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
