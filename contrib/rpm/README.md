...
The RPM spec file provided here is for EutoCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://eutocoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[EutoCoin git repository](https://github.com/EutoCoinProject/EutoCoin) but are not part of
...
Currently only `Source10` is used, for the example `eutocoin.conf` file.
...
in the eutocoin git repository. At present only an SVG version of the EutoCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the eutocoin
...
needed when 0.12.1 is released, a proper fix is already in the EutoCoin git
master branch.
...
idea to build EutoCoin Core against a static version of that library compiled
...
The `eutocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `eutocoin-server` package. At this point, most RPM based
...
The `eutocoin-server` package should have SELinux support. How to properly do
...
EutoCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/eutocoin/). He has been
packaging EutoCoin for Fedora at least since 2012.
...
4. I renamed the `eutocoin` package that contains the Qt GUI to `eutocoin-core` as
that appears to be how the general population refers to it, in contrast to
`eutocoin-xt` or `eutocoin-classic`. I wanted to make sure the general population
